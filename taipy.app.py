...
# Variables de diseño (Edita aquí para cambios rápidos)
REFRESH_TIME = 600000  # 10 minutos
COLOR_FONDO = "#0E1117"
COLOR_AZUL = "#2b5dda"
COLOR_NARANJA = "#FFBB4D"
ALT_SUP = 240
ALT_INF = 205
...
def aplicar_estilos_css():
    st.markdown(f"""
        ...
        .title-main {{ font-size: 1.4rem; font-weight: bold; margin: 0; color: {COLOR_AZUL}; }}
        .subtitle-sub {{ font-size: 0.8rem; color: #ffffff; margin: 0; }}
        .update-text {{ font-size: 0.7rem; color: {COLOR_NARANJA}; text-align: right; line-height: 1.1; }}
...
with col_sup_izq:
    try:
        ...
        fig1.update_layout(title="Tasa Overnight Diaria", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', 
                          ...
                          yaxis=dict(gridcolor='#222222', tickfont=dict(color="white")), font=dict(color=COLOR_AZUL))
...
with col_sup_der:
    try:
        ...
        fig2 = go.Figure(go.Bar(x=fechas2, y=df2.iloc[:, 1]/1000, text=[f"{v/1000:,.3f}MM" for v in df2.iloc[:, 1]], 
                                textposition='outside', marker_color=COLOR_AZUL, cliponaxis=False, 
                                textfont=dict(size=14, color="white")))
...
with col_inf_1:
    try:
        ...
        fig3 = go.Figure(go.Scatter(x=df3.iloc[:, 0], y=df3.iloc[:, 1], mode='lines+markers+text', 
                                    ...
                                    cliponaxis=False, line=dict(color=COLOR_NARANJA, width=3, shape='spline'), 
                                    textfont=dict(size=11, color="white")))
        fig3.update_layout(title="Tasa Overnight Mensual", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', 
                          height=ALT_INF, margin=dict(l=5, r=5, t=30, b=30), 
                          xaxis=dict(tickfont=dict(color="white", size=9)), 
                          yaxis=dict(showticklabels=False, gridcolor='#222222'), font=dict(color=COLOR_AZUL))
...
with col_inf_2:
    try:
        ...
        fig4 = go.Figure()
        fig4.add_trace(go.Bar(x=fechas4, y=montos4, text=[f"{v:,.1f}MM" for v in montos4], textposition='inside', 
                              marker_color='#4D79FF', textfont=dict(color="white", size=10)))
        escala4 = montos4.max() / (var4.abs().max() if var4.abs().max() != 0 else 1)
        fig4.add_trace(go.Scatter(x=fechas4, y=var4 * escala4 * 0.6, mode='lines+markers+text', 
                                  text=[f"{v:.2f}%" for v in var4], textposition="bottom center", 
                                  line=dict(color=COLOR_NARANJA, width=2), marker=dict(size=6, color='white'), 
                                  textfont=dict(color=COLOR_NARANJA, size=10)))
        fig4.update_layout(title="Base Monetaria", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', 
                          height=ALT_INF, margin=dict(l=5, r=5, t=30, b=30), 
                          xaxis=dict(tickfont=dict(color="white", size=9)), 
                          yaxis=dict(showticklabels=False, range=[montos4.min()*-0.2, montos4.max()*1.2]), 
                          font=dict(color=COLOR_AZUL), showlegend=False)
...
with col_inf_3:
    try:
        ...
        fig5 = go.Figure()
        fig5.add_trace(go.Bar(x=fechas5, y=montos5, text=[f"{int(v):,}MM" for v in montos5], textposition='inside', 
                              marker_color='#60CCC8', textfont=dict(color="white", size=11)))
        escala5 = montos5.max() / (var5.abs().max() if var5.abs().max() != 0 else 1)
        fig5.add_trace(go.Scatter(x=fechas5, y=var5 * escala5 * 0.6, mode='lines+markers+text', 
                                  text=[f"{v:.2f}%" for v in var5], textposition="bottom center", 
                                  line=dict(color=COLOR_NARANJA, width=2), marker=dict(size=6, color='white'), 
                                  textfont=dict(color=COLOR_NARANJA, size=11)))
        fig5.update_layout(title="Liquidez Monetaria", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', 
                          height=ALT_INF, margin=dict(l=5, r=5, t=30, b=30), 
                          xaxis=dict(tickfont=dict(color="white", size=9)), 
                          yaxis=dict(showticklabels=False, range=[montos5.min()*-0.2, montos5.max()*1.2]), 
                          font=dict(color=COLOR_AZUL), showlegend=False)
...
with col_inf_4:
    try:
        ...
        fig6 = go.Figure()
        fig6.add_trace(go.Bar(x=fechas6, y=montos6, text=[f"{int(v):,}MM" for v in montos6], textposition='inside', 
                              marker_color='#9E7BFF', textfont=dict(color="white", size=11)))
        escala6 = montos6.max() / (var6.abs().max() if var6.abs().max() != 0 else 1)
        fig6.add_trace(go.Scatter(x=fechas6, y=var6 * escala6 * 0.6, mode='lines+markers+text', 
                                  text=[f"{v:.2f}%" for v in var6], textposition="bottom center", 
                                  line=dict(color=COLOR_NARANJA, width=2), marker=dict(size=6, color='white'), 
                                  textfont=dict(color=COLOR_NARANJA, size=11)))
        fig6.update_layout(title="Reservas Internacionales $", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', 
                          height=ALT_INF, margin=dict(l=5, r=5, t=30, b=30), 
                          xaxis=dict(tickfont=dict(color="white", size=9)), 
                          yaxis=dict(showticklabels=False, range=[montos6.min()*-0.2, montos6.max()*1.2]), 
                          font=dict(color=COLOR_AZUL), showlegend=False)
